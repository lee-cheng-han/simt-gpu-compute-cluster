...
| Kernel done requires all warps exited and complete machine drain | Initial drain logic in `rtl/core/single_warp_core.sv`; full controller planned | `tb/integration/tb_single_warp_core.sv` | Full done/quiescence properties pending | Final event × outstanding structure pending | Partial: initial single-warp drain verified |
| EXIT deactivates only its effective lane mask | Initial lane-exit logic in single-warp core; SIMT controller planned | Full-mask EXIT in single-warp integration; predicated/divergent tests pending | Mask/deferred-path properties pending | Exit mask × stack state pending | Partial: full-mask lane exit verified |
