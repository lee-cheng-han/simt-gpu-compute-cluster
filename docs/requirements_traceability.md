...
| Stale epochs have no architectural or memory side effects | Epoch controller and all commit points | Clear with operations in flight | Stale-side-effect exclusion | Source × stale cancellation | Planned |
...
| Fatal faults immediately block issue/commit and use a dedicated sticky path | Fault controller | Fault during ALU/multiply/memory backpressure | Post-fault side-effect exclusion | Fault class × outstanding source | Planned |
